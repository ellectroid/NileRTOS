...
uint32_t task1_init() {
	...
	new_task->stack_size = TASK1_STACKSPACE_SIZE;
	new_task->stack_base_address = TASK1_STACKSPACE_BASE;
	...
	tcb_set_fpu_config(new_task, true, true);
	...
	new_task_startup_frame.startup_pc = (uint32_t) task1;
	for (int i = 0; i < 32; i++) {
		new_task_startup_frame.startup_gp_reg[i] = 0 + i;
		new_task_startup_frame.startup_fpu_reg_f[i] = 100.0f + (float) i;
	}
