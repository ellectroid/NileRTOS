...
uint32_t task0_init() {
	nile_kernel_tcb *const new_task = kalloc(sizeof(nile_kernel_tcb), 8);
	if (!new_task) {
		return 0xFFFFFFFF;
	}
	for (uint8_t *it = (uint8_t*) new_task; it < ((uint8_t*) new_task + sizeof(nile_kernel_tcb)); it++) {
		*it = 0x00;
	}

	new_task->task_id = 0;
	new_task->scheduling_priority_base = 10;
	...
	new_task->stack_size = TASK0_STACKSPACE_SIZE;
	new_task->stack_base_address = TASK0_STACKSPACE_BASE;
	...
	new_task->memprot_regions[2].base = NILE_QUADSPI_MEMMAPPED_ADDR;          // QSPI Flash read
	new_task->memprot_regions[2].size = 1 << 24;                     // QSPI Flash
	new_task->memprot_regions[2].perms = NILE_MEMPROT_PERM_RO_FULL;   // full read
	new_task->memprot_regions[2].type = NILE_MEMPROT_MEM_NORMAL;     // normal cached
	new_task->memprot_regions[2].executable = false;                       // XN

	for (int i = 3; i < NILE_KERNEL_TASK_MEMPROT_REGION_COUNT; ++i) {
		new_task->memprot_regions[i].base = 0;
		new_task->memprot_regions[i].size = 0;
	...
	tcb_set_fpu_config(new_task, false, false);

	nile_kernel_tcb_startup_frame_gp new_task_startup_frame = { 0 };
	new_task_startup_frame.startup_sp = new_task->stack_base_address;
	new_task_startup_frame.startup_pc = (uint32_t) task0;
	for (int i = 0; i < 32; i++) {
		new_task_startup_frame.startup_gp_reg[i] = 1000 + i;
	}
	ctx_init_gp(new_task, &new_task_startup_frame);
